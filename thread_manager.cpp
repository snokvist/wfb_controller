 ...
 *  ────────────────────────────────────────────
 *  Drop-in replacement for the original Python script.  Feature-parity:
 *    • `[threads]`  section  →  long-lived processes with auto-restart
 *    • `[commands]` section  →  “exec <name>” one-shot commands
 *    • text TCP protocol identical to the Python version
 *
 *  Requires Boost 1.81+   (Process · Asio · PropertyTree · Beast · ProgOpt)
 *
 *  $ g++ -std=c++17 -O2 -pthread   \
 *        thread_manager.cpp        \
 *        -lboost_system -lboost_thread -lboost_filesystem \
 *        -lboost_program_options -lboost_process          \
 *        -o thread_manager
 *
 *  Author: ChatGPT — 17 May 2025   •   Licence: MIT
 */
    ...
    void stop(bool forceKill = false) {
        ...
        if (child_->running()) {
            ...
            if (forceKill) {
                ...
                if (child_->running()) { child_->terminate(); status = "KILL"; }
            }
        }
        ...
        while (!quit_) {
            ...
            for (auto& [_,p]: map_) {
                ...
                if (!p->isRunning() && p->status()!="missing") {
                    std::this_thread::sleep_for(seconds(RESTART_DELAY_SEC));
                    p->restarts++; p->start();
                } else if (p->status()=="missing") {
                    p->start();    // re-check after MISSING_RECHECK_SEC inside start()
                }
            }
        }
    }
