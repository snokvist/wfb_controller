...
typedef struct {
    ...
    uint32_t frames, bytes, rc_cnt, radio_cnt, gps_cnt;
...
typedef struct {
    ...
    uint16_t chan_mask;
    int udp_in_port;
    const char *udp_out_host; const char *udp_out_port;
    bool no_serial;
} cfg_t;

/* -------------------- HELP ------------------------------------ */
static void print_help(const char *prog)                     /* HELP */
{
    fprintf(stderr,
        "Usage: %s [options]\n"
        "\nSerial:\n"
        "  -d, --device PATH        Serial device to open (default %s)\n"
        "  -b, --baud RATE          Baud-rate (default %d)\n"
        "  -n, --no-serial          Disable the serial port completely\n"
        "\nUDP:\n"
        "  -u, --udp-in PORT        Listen for MAVLink on UDP PORT\n"
        "  -o, --udp-out HOST:PORT  Forward every frame to HOST:PORT\n"
        "\nOutput / debug:\n"
        "  -p, --period MS          Stats print period in ms (default %u)\n"
        "  -r, --raw                Dump every parsed frame in hex\n"
        "\nRC-triggered commands:\n"
        "  -c, --command-parse      Enable RC channel monitoring & script exec\n"
        "      --channels LIST      Channels to watch (1-16, comma-sep, default all)\n"
        "      --min-change VAL     Minimum delta to start detection (default 100)\n"
        "      --persist MS         Value must persist this long before exec (default 500)\n"
        "      --pause MS           Pause after exec before next one (default 500)\n"
        "\nOther:\n"
        "  -h, --help               Show this help text and exit\n",
        prog, DEF_DEV, DEF_BAUD, DEF_PER);
}                                                         /* HELP */
...
static void cfg_default(cfg_t *c)
{
    ...
    c->chan_mask = 0xFFFF;
    c->udp_in_port = 0; c->udp_out_host = c->udp_out_port = NULL; c->no_serial = false;
}
...
static void parse_udp_out(cfg_t *c, const char *s)
{
    ...
    char *h = malloc(hlen + 1); if (!h) die("malloc");
    memcpy(h, s, hlen); h[hlen] = 0;
...
static void parse_cli(int ac, char **av, cfg_t *c)
{
    ...
    int o, i;
    while ((o = getopt_long(ac, av, "d:b:p:rcu:o:nh", L, &i)) != -1)   /* HELP */
        switch (o) {
            ...
            case 'h': print_help(av[0]); exit(0);          /* HELP */
            ...
            default:  print_help(av[0]); exit(1);          /* HELP */
        }

    if (c->no_serial) c->dev = NULL;
    if (!c->dev && !c->udp_in_port && !c->udp_out_host) {
        fprintf(stderr, "At least one of serial, udp-in or udp-out must be configured\n");
        exit(1);
    }
}
...
static int open_udp_in(int port)
{
    int fd = socket(AF_INET, SOCK_DGRAM, 0); if (fd < 0) die("socket in");
    int yes = 1; setsockopt(fd, SOL_SOCKET, SO_REUSEADDR, &yes, sizeof yes);
    struct sockaddr_in a = {.sin_family = AF_INET, .sin_addr.s_addr = htonl(INADDR_ANY), .sin_port = htons(port)};
    if (bind(fd, (struct sockaddr *)&a, sizeof a) < 0) die("bind in");
...
static int open_udp_out(const char *host, const char *port)
{
    ...
    struct addrinfo *res;
    int r = getaddrinfo(host, port, &hint, &res);
    if (r != 0) { fprintf(stderr, "udp-out getaddrinfo: %s\n", gai_strerror(r)); exit(1); }
    int fd = socket(res->ai_family, res->ai_socktype, res->ai_protocol);
    if (fd < 0) die("socket out");
    if (connect(fd, res->ai_addr, res->ai_addrlen) < 0) die("connect out");
    freeaddrinfo(res);
    return mk_nonblock(fd);
}
...
int main(int argc, char **argv)
{
    ...
    for (;;) {
        struct pollfd pfds[3]; int nfds = 0;
        if (fd_serial >= 0) pfds[nfds++] = (struct pollfd){.fd = fd_serial, .events = POLLIN};
        if (fd_udp_in >= 0) pfds[nfds++]  = (struct pollfd){.fd = fd_udp_in,  .events = POLLIN};
        int pr = poll(pfds, nfds, 50);
        if (pr < 0 && errno != EINTR) pr = 0;

        /* ---- handle readable fds ---- */
        for (int i = 0; i < nfds; ++i) if (pfds[i].revents & POLLIN) {
            int fd = pfds[i].fd;
            ...
            if (n <= 0) {
                if (fd == fd_serial && (n == 0 || (errno != EAGAIN && errno != EINTR))) {
                    ...
                    if (cfg.dev) {
                        ...
                        close(fd_serial); fd_serial = -1; sleep(3);
                        fd_serial = open_serial(&cfg);
                    }
                }
                continue;
            }
            ...
            if (fd_udp_out >= 0 && n > 0) {
                ssize_t wr = send(fd_udp_out, buf, n, 0);
                if (wr > 0) st.udp_tx += (uint32_t)wr;
            }
            ...
            for (ssize_t ix = 0; ix < n; ++ix) if (parse_byte(buf[ix], &f)) {
                ...
                case ID_RC_OVERRIDE: {
                    ...
                    if (cfg.cmd_en) {
                        uint64_t now = ms_now();
                        for (int cix = 0; cix < 16; ++cix) if (cfg.chan_mask & (1u << cix)) {
                            int val = vals[cix];
                            ch_state_t *s = &ch[cix];
                            ...
                            if (s->pending) {
                                ...
                                if (now - s->start_ms >= cfg.persist_ms &&
                                    ...
                                    st.execs[st.exec_cnt++] =
                                        (typeof(st.execs[0])){cix + 1, s->old_val, val, rt};
                ...
                case ID_GLOBAL_POSITION_INT:
                    if (f.len < LEN_GPS_MIN) break;
                    st.gps_cnt++;
                    st.gps_time_ms = *(const uint32_t *)(f.pay + 0);
        ...
        uint64_t now = ms_now();
        if (now >= st.next_ms) {
            printf("%" PRIu64 " MAVLINK_STATS %u:%u:%u:%u:%u:%u:%u\n",
                   now, cfg.period, st.frames,
                   (st.rc_cnt ? 1:0) + (st.radio_cnt ? 1:0) + (st.gps_cnt ? 1:0),
                   st.bytes, st.udp_rx, st.udp_tx, st.gps_cnt);
